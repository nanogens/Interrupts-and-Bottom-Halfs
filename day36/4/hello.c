...
// step 3 -- this is called when softirq is pending
...
// this is the interrupt handler that fires when the button is pressed.
static irqreturn_t  button_handler(int irq, void *dev_id)
{
        pr_info("irq:%d\n", irq);
	raise_softirq(MY_SOFTIRQ);  // step 2 -- this will mark the softirq as pending.  when it executes, it will call my_actions()
        return IRQ_HANDLED;
}
...
static int test_hello_init(void)
{
	...
	open_softirq(MY_SOFTIRQ, my_action);  // step 1 -- is to register your softirq handler.  
